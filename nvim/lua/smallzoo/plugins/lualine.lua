...
return {
	...
	-- $XDG_DATA_HOME/nvim/lazy/LazyVim/lua/lazyvim/plugins/ui.lua
	{
		...
		opts = function()
			...
			local ColorUtil = require('smallzoo.util.color')
			local theme = {
				normal = {
					a = active,
					b = active,
			...
			local opts = {
				...
				sections = {
					lualine_a = {
						...
						{
							require('smallzoo.util.lualine').plugin_title(),
					...
					lualine_b = {
						...
						-- stylua: ignore
						{
							require('smallzoo.util.lualine').trails(),
							cond = is_file_window,
							padding = { left = 1, right = 0 },
					...
					lualine_y = {
						{
							require('smallzoo.util.lualine').filemedia(),
							padding = 1,
							cond = function()
