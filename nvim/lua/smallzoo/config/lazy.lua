-- Smallzoo's lazy.nvim initialization
...
-- Load smallzoo.config.* after lazyvim's base files:
...
for _, name in pairs({ 'options', 'autocmds', 'keymaps' }) do
	vim.api.nvim_create_autocmd('User', {
		group = vim.api.nvim_create_augroup('smallzoo.' .. name, { clear = true }),
		pattern = 'LazyVim' .. name:sub(1, 1):upper() .. name:sub(2) .. 'Defaults',
		once = true,
		callback = function()
			require('smallzoo.config').load(name)
		end,
	})
end
