-- smallzooDev's Neovim config loader
...
M.deprecated_extras = {
	['smallzooDev.plugins.extras.editor.telescope'] = '`telescope.nvim` is now the default **smallzooDevVim** picker.',
	['smallzooDev.plugins.extras.ui.indent-blankline'] = "Use LazyVim's `indent-blankline.nvim` instead.",
}

-- Load lua/smallzooDev/config/* and user lua/config/* files.
---@param name 'autocmds' | 'options' | 'keymaps'
function M.load(name)
	local mod = 'smallzooDev.config.' .. name
	...
	if name == 'options' then
		require('smallzooDev.config').setup()
	end
end
...
function M.setup()
	...
	-- Add lua/smallzooDev/plugins/extras as list of "extra" sources.
	LazyVim.extras.sources = {
		...
		{
			name = 'smallzooDev ',
			desc = 'smallzooDev extras',
			module = 'smallzooDev.plugins.extras',
		},
		{
			name = 'User ',
			desc = 'User extras',
